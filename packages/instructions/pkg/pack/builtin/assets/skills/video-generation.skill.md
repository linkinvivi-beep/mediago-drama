...
- 使用 MediaLink MCP 完成视频生成，不要临时编写脚本或直接调用供应商接口。
...
锁定分镜目标时默认遵循**一镜一条**：一个分镜 section 对应一个独立视频任务。若正文混入多个镜头、转场或互相冲突的运镜，先按剧情节拍拆开，不要把它们作为一个长提示词提交。

提交用的基础 `prompt` 必须是可执行的单镜描述，并按内容自然覆盖：

- **主体与一致性**：要保持的人物、服饰、场景、道具及参考图约束。
- **起始状态**：开镜时的主体位置、姿态、表情、空间关系和环境状态。
- **主要动作**：一个主要叙事动作，写清动作过程和结果。
- **主要镜头运动**：只保留一种主要运镜，并明确景别、机位、方向；静态镜头要明确写出。
- **结束状态**：镜头最终停留的动作状态、主体位置和构图，便于剪辑或下一镜衔接。
- **环境与声音**：与本镜相关的光线、环境动态、环境音、动作音效和必要对白。

不要在 `prompt` 中编造时长、比例、分辨率、帧率或模型专属语法；这些由统一生成设置表单和选定 route 的实时 schema 决定。提示词优化只能改善表达，不能把一镜扩写成多镜、改变剧情事实，或覆盖参考素材。
...
目标是含多个独立镜头的分镜文档（正文用 `## 第 0N 组` 二级标题组织，每组恰好一镜）时按本节执行。本节替代标准流程的第 1、3、4 步；第 2 步统一生成设置表单仍然共用。
...
- `items` 中每个镜头组一个请求：
  ...
  - `prompt` 取该镜头组的一镜描述，保持“主体与一致性 → 起始状态 → 主要动作 → 主要镜头运动 → 结束状态 → 环境与声音”的信息顺序，并沿用整批统一的风格与一致性要求。
